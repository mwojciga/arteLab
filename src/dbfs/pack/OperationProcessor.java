...
public class OperationProcessor implements SerialPortEventListener {
	 ...
	 * @param error
	 */
	public void writeData(int rightRelayTime, int leftRelayTime, int error) {
		try {
			String rightStr = String.format("%03d", rightRelayTime);
			String leftStr = String.format("%03d", leftRelayTime);
			//Sr000l000e0E
			//S_RIGHTPUMP_LEFTPUMP_ERROR_E
			// Send the message.
			outputStream.flush();
			String toSend = "S".concat("r" + rightStr + "l" + leftStr + "e" + error + "E");
			...
			if (error == 0) {
				...
				log.log("Relays stopped by user!", "Relays stopped by user!");
				logger.info("Relays stopped by user!");
			}
		} catch (Exception e) {
